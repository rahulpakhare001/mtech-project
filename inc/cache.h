...
#define IS_PTW  7
#ifdef PUSH_DTLB_PB
#define IS_DTLB_PB 13
#endif
#ifdef PUSH_VICTIMS_DTLB_VB
#define IS_DTLB_VB 14
#endif
#ifdef PUSH_DTLB_UB
#define IS_DTLB_UB 15
#endif
#ifdef DTLB_NC_BUFFER
#define IS_DTLB_NC 16
#endif

#ifdef ARKA_DP_PRED
#define IS_VICTIM_ST 17
#endif
...
/*#ifdef CRITICAL_TLB
#define DTLB_CRITICAL_WAY 2
#endif*/

//@Vasudha: DTLB Prefetch Buffer
#ifdef PUSH_DTLB_PB
#define DTLB_PB_SET 1
#define DTLB_PB_WAY 16
...
//@Vasudha: DTLB Victim Buffer
#ifdef PUSH_VICTIMS_DTLB_VB
#define DTLB_VB_SET 1
#define DTLB_VB_WAY 16
#define DTLB_VB_RQ_SIZE 0
#define DTLB_VB_WQ_SIZE 0
#define DTLB_VB_PQ_SIZE 0
#define DTLB_VB_MSHR_SIZE 0
#endif

//@Vasudha : DTLB Utility Buffer
#ifdef PUSH_DTLB_UB
#define DTLB_UB_SET 1
#define DTLB_UB_WAY 8
#define DTLB_UB_RQ_SIZE 0
#define DTLB_UB_WQ_SIZE 0
#define DTLB_UB_PQ_SIZE 0
#define DTLB_UB_MSHR_SIZE 0
#endif
...
/*#ifdef CRITICAL_TLB
#define STLB_CRITICAL_WAY 6
#endif*/
#ifdef H_D_H_P
#define STLB_HALF_WAY 6
#endif

#ifdef ARKA_DP_PRED
// VICTIM SHADOW TABLE
#define VICTIM_ST_SET 128
#define VICTIM_ST_WAY 12
#endif
...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t sim_access[NUM_CPUS][MAX_THREADS][NUM_TYPES],
             sim_hit[NUM_CPUS][MAX_THREADS][NUM_TYPES],
             sim_miss[NUM_CPUS][MAX_THREADS][NUM_TYPES],
             roi_access[NUM_CPUS][MAX_THREADS][NUM_TYPES],
             roi_hit[NUM_CPUS][MAX_THREADS][NUM_TYPES],
             roi_miss[NUM_CPUS][MAX_THREADS][NUM_TYPES];

    uint64_t total_miss_latency;

//    uint64_t sim_miss_latency[NUM_CPUS][MAX_THREADS][NUM_TYPES],
//             roi_miss_latency[NUM_CPUS][MAX_THREADS][NUM_TYPES];
...
#ifdef PT_STATS
    uint64_t sim_pt_access[NUM_CPUS][MAX_THREADS][5], sim_pt_hit[NUM_CPUS][MAX_THREADS][5], sim_pt_miss[NUM_CPUS][MAX_THREADS][5], 
	     roi_pt_access[NUM_CPUS][MAX_THREADS][5], roi_pt_hit[NUM_CPUS][MAX_THREADS][5], roi_pt_miss[NUM_CPUS][MAX_THREADS][5];
#endif
...
*/
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
	    ...
	    for (uint8_t thread_num = 0; thread_num < MAX_THREADS; thread_num++)
	    {
            	for (uint32_t j=0; j<NUM_TYPES; j++) {
                	sim_access[i][thread_num][j] = 0;
                	sim_hit[i][thread_num][j] = 0;
                	sim_miss[i][thread_num][j] = 0;
                	roi_access[i][thread_num][j] = 0;
                	roi_hit[i][thread_num][j] = 0;
                	roi_miss[i][thread_num][j] = 0;
			
		//	sim_miss_latency[i][thread_num][j] = 0;
		//	roi_miss_latency[i][thread_num][j] = 0;
            	}
	    }
        }
		...
		for (uint32_t k = 0; k < 5; k++) {
        		sim_pt_access[i][j][k] = 0;
		        sim_pt_hit[i][j][k] = 0;
		        sim_pt_miss[i][j][k] = 0;
			roi_pt_access[i][j][k] = 0;
			roi_pt_hit[i][j][k] = 0;
		        roi_pt_miss[i][j][k] = 0;
		}
	    }
...
//@Rahul: calculating dead blocks
    ...
    int  check_hit(PACKET *packet, uint32_t set),
         invalidate_entry(uint64_t inval_addr, uint8_t thread),
         ...
         prefetch_translation(uint64_t ip, uint64_t pf_addr, int pf_fill_level, uint64_t prefetch_metadata, uint64_t prefetch_id, uint8_t instruction),
	...
	#ifdef ARKA_DP_PRED
		 //victim_st_initialize_replacement(),
	#endif
	...
	#ifdef ARKA_DP_PRED		
		//victim_st_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit),
	#endif
	...
	#ifdef ARKA_DP_PRED
	victim_st_replacement_final_stats(),
	#endif
         ...
         dtlb_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint64_t prefetch_id, uint8_t instruction, uint8_t critical_ip_flag),
	 ...
	 l1d_prefetcher_notify_about_dtlb_eviction(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in),
         ...
         //dtlb_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in, uint16_t thread_ID),
	 ...
	 dtlb_prefetcher_send_feedback(uint64_t, uint64_t),
	 stlb_prefetcher_send_feedback(uint64_t, uint64_t, uint64_t),
	 dtlb_victim_buffer_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in, uint16_t thread_ID);
	...
	void (*l1i_prefetcher_cache_fill)(uint32_t, uint64_t, uint32_t, uint32_t, uint8_t, uint64_t);
         ...
         uint64_t dtlb_prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in, uint16_t thread_ID);
    ...
    uint32_t get_set(uint64_t address),
             get_way(uint64_t address, uint32_t set, uint64_t instr_id),
         ...
         //victim_st_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
#endif
