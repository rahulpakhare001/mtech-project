...
class PAGE_TABLE_WALKER : public MEMORY {
  public:
    ...
    uint64_t next_translation_virtual_address[MAX_THREADS];	// = 0xf000000f00000000; //Stores the virtual address from which translation pages will reside

	queue <uint64_t > page_queue;
	map <uint64_t, uint64_t> inverse_table, recent_page;
	map <uint64_t, map <uint64_t, uint64_t>> page_table;
	uint64_t previous_ppage, num_adjacent_page, allocated_pages, num_page[NUM_CPUS][MAX_THREADS], major_fault[NUM_CPUS][MAX_THREADS], minor_fault[NUM_CPUS][MAX_THREADS];
    ...
    PAGE_TABLE_PAGE *L5[MAX_THREADS]; //CR3 register points to the base of this page.
    uint64_t CR3_addr[MAX_THREADS]; //This address will not have page offset bits.
    bool CR3_set[MAX_THREADS];
	...
	{
		CR3_addr[thread_num] = UINT64_MAX;
		CR3_set[thread_num] = false;
		L5[thread_num] = NULL;
		next_translation_virtual_address[thread_num] = 0xf000000f00000000 | (thread_num << 7);
	}

	for (int i=0; i < NUM_CPUS; i++)
		for (int thread_num = 0; thread_num < MAX_THREADS; thread_num++)
			num_page[i][thread_num] = 0;
        rq_full = 0;
    ...
    uint64_t get_index(uint64_t address, uint8_t cache_type),
             check_hit(CACHE &cache, uint64_t address, uint8_t type, uint64_t instr_id),
             get_offset(uint64_t address, uint8_t pt_level),
             handle_page_fault(PAGE_TABLE_PAGE* page, PACKET *packet, uint8_t pt_level);

	uint64_t map_translation_page(bool *page_swap, uint64_t instr_id),
