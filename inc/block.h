...
class BLOCK {
  public:
    ...
    uint16_t critical;	//@Vasudha: To know whether translation stored in TLB is critical or not
...
class PACKET_QUEUE {
  public:
    ...
    // functions
    int check_queue(PACKET* packet, uint8_t cache_type);
    void add_queue(PACKET* packet),
         remove_queue(PACKET* packet);
};

// reorder buffer
class PARTITIONED_CORE_BUFFER {
  public:
    const string NAME;
    const uint32_t SIZE;
    uint32_t cpu, 
             head[MAX_THREADS],	//@Vasudha:SMT: Every logical partition of buffer will comprise of its own head and tail 
             tail[MAX_THREADS],
	     partition[MAX_THREADS],	//@Vasudha:SMT: Stores the limit of a partition
             occupancy[MAX_THREADS],
             last_read, last_fetch, last_scheduled, 
             inorder_fetch[2],
             next_fetch[2],
             next_schedule[MAX_THREADS];
    ...
    // constructor
    PARTITIONED_CORE_BUFFER(string v1, uint32_t v2) : NAME(v1), SIZE(v2) {
        
        for (int thread_num=0; thread_num < MAX_THREADS; thread_num++)
	{
		head[thread_num] = 0;
		tail[thread_num] = 0;
		partition[thread_num] = 0;
		occupancy[thread_num] = 0;
        	next_schedule[thread_num] = 0;
	}
    ...
    // destructor
    ~PARTITIONED_CORE_BUFFER() {
        delete[] entry;
    };
};


// IFETCH BUFFER, DECODE BUFFER
class CORE_BUFFER {
  public:
    const string NAME;
    const uint32_t SIZE;
