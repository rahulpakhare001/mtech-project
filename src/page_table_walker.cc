...
set <uint64_t> all_pscl2_entry[NUM_CPUS][MAX_THREADS]; 
...
void PAGE_TABLE_WALKER::operate()
{
	...
	if(MSHR.occupancy > 0) //Handle pending request, only one request is serviced at a time.
	{
		...
		{
			int index = MSHR.head;
			int thread_num = ((1 << LOG2_THREADS) - 1) & MSHR.entry[index].instr_id;
		
			if (knob_cloudsuite)
				thread_num = 0;	
			assert(CR3_addr[thread_num] != UINT64_MAX);
			PAGE_TABLE_PAGE* curr_page = L5[thread_num]; //Start wth the L5 page
			...
			for (int i = 5; i > MSHR.entry[index].translation_level; i--)
			{
				...
				if(next_level_base_addr == UINT64_MAX)
				{
					if(MSHR.entry[index].type == PREFETCH_TRANSLATION || MSHR.entry[index].type == TRANSLATION_FROM_L1D)
					{
						...
						ooo_cpu[cpu].total_drop_from_PTW++;
						...
						#ifdef DEBUG_PREF 
						if (all_warmup_complete > 2)
							cout << "DROPPED PREFETCH: " << hex << MSHR.entry[index].address << dec << endl;
						#endif
			...
			if(MSHR.entry[index].translation_level == 0 && !dropped_prefetch_request) //If translation complete
			{
				...
				if (knob_cloudsuite)
				{
						if(MSHR.entry[index].instruction)
						{
							MSHR.entry[index].address = (( MSHR.entry[index].ip >> LOG2_PAGE_SIZE) << 9) | ( 256 + MSHR.entry[index].asid[0]);	
						}
						else
						{
							MSHR.entry[index].address = (( MSHR.entry[index].full_virtual_address >> LOG2_PAGE_SIZE) << 9) |  MSHR.entry[index].asid[1];
	...
	else if(RQ.occupancy > 0) //If there is no pending request which is undergoing translation, then process new request.
	{
		...
		//if((RQ.entry[RQ.head].event_cycle <= current_core_cycle[cpu]) && (ooo_cpu[cpu].L2C.RQ.occupancy < ooo_cpu[cpu].L2C.RQ.SIZE))
		{
			...
			uint64_t address_pscl5 = check_hit(PSCL5,get_index(RQ.entry[index].full_addr,IS_PSCL5),RQ.entry[index].type,RQ.entry[index].instr_id);
			uint64_t address_pscl4 = check_hit(PSCL4,get_index(RQ.entry[index].full_addr,IS_PSCL4),RQ.entry[index].type,RQ.entry[index].instr_id);
			uint64_t address_pscl3 = check_hit(PSCL3,get_index(RQ.entry[index].full_addr,IS_PSCL3),RQ.entry[index].type,RQ.entry[index].instr_id);
			uint64_t address_pscl2 = check_hit(PSCL2,get_index(RQ.entry[index].full_addr,IS_PSCL2),RQ.entry[index].type,RQ.entry[index].instr_id);

			if (all_warmup_complete > ooo_cpu[cpu].thread)
				all_pscl2_entry[cpu][((1<<LOG2_THREADS)-1) & RQ.entry[index].instr_id].insert(RQ.entry[index].full_addr >> 21);
		...
		next_address = address_pscl5 << LOG2_PAGE_SIZE | (get_offset(RQ.entry[index].full_addr,IS_PTL4) << 3);				
            ...
            {
		int thread_num = ((1 << LOG2_THREADS) - 1) & RQ.entry[index].instr_id;    
            	
		if (knob_cloudsuite)
			thread_num = 0;
		if(CR3_addr[thread_num] == UINT64_MAX)
            	{
            		assert(!CR3_set[thread_num]); //This should be called only once when the process is starting
            		handle_page_fault(L5[thread_num], &RQ.entry[index], 6); //6 means first level is also not there
            		CR3_set[thread_num] = true;

            		PAGE_TABLE_PAGE* curr_page = L5[thread_num];
					uint64_t next_level_base_addr = UINT64_MAX;
            	...
            	next_address = CR3_addr[thread_num] << LOG2_PAGE_SIZE | (get_offset(RQ.entry[index].full_addr,IS_PTL5) << 3);				
...
//	    packet.instruction = 0;
	...
	else if(PQ.occupancy > 0) 
	{
		//@Rahul: PTW
	        ...
	        #ifdef PTW_L1D_L2C
	    	if((PQ.entry[PQ.head].event_cycle <= current_core_cycle[cpu]) && (ooo_cpu[cpu].L1D.RQ.occupancy < ooo_cpu[cpu].L1D.RQ.SIZE) && (ooo_cpu[cpu].L2C.RQ.occupancy < ooo_cpu[cpu].L2C.RQ.SIZE))
                #endif
		...
		//if((PQ.entry[PQ.head].event_cycle <= current_core_cycle[cpu]) && (ooo_cpu[cpu].L2C.RQ.occupancy < ooo_cpu[cpu].L2C.RQ.SIZE))
                 {
                         ...
                         uint64_t address_pscl5 = check_hit(PSCL5,get_index(PQ.entry[index].full_addr,IS_PSCL5),PQ.entry[index].type,PQ.entry[index].instr_id);
                         uint64_t address_pscl4 = check_hit(PSCL4,get_index(PQ.entry[index].full_addr,IS_PSCL4),PQ.entry[index].type,PQ.entry[index].instr_id);
                         uint64_t address_pscl3 = check_hit(PSCL3,get_index(PQ.entry[index].full_addr,IS_PSCL3),PQ.entry[index].type,PQ.entry[index].instr_id);
			 uint64_t address_pscl2 = check_hit(PSCL2,get_index(PQ.entry[index].full_addr,IS_PSCL2),PQ.entry[index].type,PQ.entry[index].instr_id);
		     ...
		     int thread_num = ((1 << LOG2_THREADS) - 1) & PQ.entry[index].instr_id;  
		     
		     if (knob_cloudsuite)
			     thread_num = 0;
		     if(CR3_addr[thread_num] == UINT64_MAX)
		     {
			//while prefetching, CR3_set should be true
			assert(CR3_set[thread_num]); //This should be called only once when the process is starting 
                        handle_page_fault(L5[thread_num], &PQ.entry[index], 6); //6 means first level is also not there
		     ...
		     next_address = CR3_addr[thread_num] << LOG2_PAGE_SIZE | (get_offset(PQ.entry[index].full_addr,IS_PTL5) << 3);
                     packet.translation_level = 5;
	     }
...
uint64_t PAGE_TABLE_WALKER::handle_page_fault(PAGE_TABLE_PAGE* page, PACKET *packet, uint8_t pt_level)
{
	...
	int thread_num = ((1 << LOG2_THREADS) - 1) & packet->instr_id;
	
	if (knob_cloudsuite)
		thread_num = 0;
	if(pt_level == 6)
	{
		assert(page == NULL && CR3_addr[thread_num] == UINT64_MAX);
		L5[thread_num] = new PAGE_TABLE_PAGE();
		CR3_addr[thread_num] = map_translation_page(&page_swap, packet->instr_id);
		pt_level--;
		write_translation_page(CR3_addr[thread_num], packet, pt_level);
		page = L5[thread_num];
	}
	...
	while(pt_level > 1)
	{
		...
		page->next_level_base_addr[offset] = map_translation_page(&page_swap, packet->instr_id);
...
uint64_t PAGE_TABLE_WALKER :: va_to_pa_ptw(uint8_t cpu, uint64_t instr_id, bool translation_page, uint64_t va, uint64_t unique_vpage, bool *page_swap)
{
    ...
    //@Vasudha:SMT: search page table corresponding to thread
    auto pr = page_table[thread_num].find(vpage);
    auto ppage_check = inverse_table.begin();

    if (pr == page_table[thread_num].end()) { // no VA => PA translation found 
        ...
        if (allocated_pages >= DRAM_PAGES) { // not enough memory
            ...
            for (pr = page_table[thread_num].begin(); pr != page_table[thread_num].end(); pr++) {
            ...
            if (pr == page_table[thread_num].end())
                assert(0);
#endif
            ...
            page_table[thread_num].erase(pr);
            page_table[thread_num].insert(make_pair(vpage, mapped_ppage));
            ...
            ooo_cpu[cpu].ITLB.invalidate_entry(NRU_vpage, thread_num);
            ooo_cpu[cpu].DTLB.invalidate_entry(NRU_vpage, thread_num);
            ooo_cpu[cpu].STLB.invalidate_entry(NRU_vpage, thread_num);
            for (uint32_t i=0; i<BLOCK_SIZE; i++) {
                uint64_t cl_addr = (mapped_ppage << 6) | i;
                ooo_cpu[cpu].L1I.invalidate_entry(cl_addr, thread_num);
                ooo_cpu[cpu].L1D.invalidate_entry(cl_addr, thread_num);
                ooo_cpu[cpu].L2C.invalidate_entry(cl_addr, thread_num);
                uncore.LLC.invalidate_entry(cl_addr, thread_num);
            }
            ...
            *page_swap = true;
            ...
            page_table[thread_num].insert(make_pair(vpage, random_ppage));
            inverse_table.insert(make_pair(random_ppage, vpage));
            page_queue.push(vpage);
            previous_ppage = random_ppage;
            num_adjacent_page--;
            num_page[cpu][thread_num]++;
            allocated_pages++;
        ...
        if (*page_swap)
            major_fault[cpu][thread_num]++;
        else
            minor_fault[cpu][thread_num]++;
    }
    else {
        //printf("Found  vpage: %lx  random_ppage: %lx\n", vpage, pr->second);
    }

    pr = page_table[thread_num].find(vpage);
#ifdef SANITY_CHECK
    if (pr == page_table[thread_num].end())
        assert(0);
#endif
    ...
    DP ( if (warmup_complete[cpu][thread_num]) {
    cout << "[PAGE_TABLE] instr_id: " << (instr_id >> LOG2_THREADS) << " vpage: " << hex << vpage;
	...
	#if defined(PERFECT_DTLB) || defined(PERFECT_STLB)
	uint64_t pa;
	auto it = ooo_cpu[cpu].PTW.page_table[thread_num].find(unique_vpage);
        assert(it != ooo_cpu[cpu].PTW.page_table[thread_num].end());
	pa = it->second << LOG2_PAGE_SIZE;
	#endif
    return pa;
}

uint64_t PAGE_TABLE_WALKER::map_translation_page(bool *page_swap, uint64_t instr_id)
{
	uint8_t thread_num = ((1 << LOG2_THREADS) - 1) & instr_id;
	if (knob_cloudsuite)
		thread_num = 0;
	uint64_t physical_address = va_to_pa_ptw(cpu, 0, true , next_translation_virtual_address[thread_num], next_translation_virtual_address[thread_num] >> LOG2_PAGE_SIZE, page_swap);
	next_translation_virtual_address[thread_num] = ( (next_translation_virtual_address[thread_num] >> LOG2_PAGE_SIZE) + 1 ) << LOG2_PAGE_SIZE;
...
void PAGE_TABLE_WALKER::fill_mmu_cache(CACHE &cache, uint64_t next_level_base_addr, PACKET *packet, uint8_t cache_type)
{
    ...
    cache.MSHR.entry[0].critical_ip_flag = packet->critical_ip_flag;
...
uint64_t PAGE_TABLE_WALKER::check_hit(CACHE &cache, uint64_t address, uint8_t type, uint64_t instr_id)
{
	...
	//@Vasudha:SMT: Before searching translations, check thread-ID that is appended to instruction_id
	if ((((1 << LOG2_THREADS) -1) & cache.block[set][way].instr_id) == (((1 << LOG2_THREADS) - 1) & instr_id) || knob_cloudsuite)
	{
        	if (cache.block[set][way].valid && (cache.block[set][way].tag == address)) 
		{
		    // COLLECT STATS
        	    cache.sim_hit[cpu][((1 << LOG2_THREADS) - 1) & instr_id][type]++;
            	    cache.sim_access[cpu][((1 << LOG2_THREADS) - 1) & instr_id][type]++;
		    cache.block[set][way].used = 1;
...
int  PAGE_TABLE_WALKER::add_rq(PACKET *packet)
{
    ...
    int index = RQ.check_queue(packet, cache_type);
...
void PAGE_TABLE_WALKER::return_data(PACKET *packet)
{
	...
	if (MSHR.entry[index].address == packet->address) {
	   //@Vasudha:SMT: check thread-ID of packets
	   if ((((1 << LOG2_THREADS) - 1) & MSHR.entry[index].instr_id) == (((1 << LOG2_THREADS) - 1) & packet->instr_id) || knob_cloudsuite)
	   {
		mshr_index = index;
	   	break;
	    }
