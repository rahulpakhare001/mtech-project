...
void MEMORY_CONTROLLER::process(PACKET_QUEUE *queue)
{
    ...
    if (bank_request[op_channel][op_rank][op_bank].cycle_available <= current_core_cycle[op_cpu]) {
		...
		//@Rahul: PTW
#if defined(PTW_DRAM) && defined(INS_PAGE_TABLE_WALKER)
		if (queue->entry[request_index].type == LOAD_TRANSLATION || queue->entry[request_index].type == PREFETCH_TRANSLATION || queue->entry[request_index].type == TRANSLATION_FROM_L1D)
		    PTW_interface[op_cpu]->return_data(&queue->entry[request_index]);
		else
...
int MEMORY_CONTROLLER::add_rq(PACKET *packet)
{
    ...
    if (all_warmup_complete < total_threads) {
        ...
        {
              PTW_interface[packet->cpu]->return_data(packet);
              return -1;
        }
#endif
