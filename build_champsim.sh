...
#thread[0]=${10:-${thread[0]}}
#thread[1]=${11:-${thread[1]}}
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${ITLB_PREFETCHER}-${DTLB_PREFETCHER}-${STLB_PREFETCHER}-${BTB_REPLACEMENT}-${L1I_REPLACEMENT}-${L1D_REPLACEMENT}-${L2C_REPLACEMENT}-${LLC_REPLACEMENT}-${ITLB_REPLACEMENT}-${DTLB_REPLACEMENT}-${STLB_REPLACEMENT}-${NUM_CORE}core"
#BINARY_NAME="${BRANCH}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${ITLB_PREFETCHER}-${DTLB_PREFETCHER}-${STLB_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core"
#cpu_no=0
#while [ ${cpu_no} -lt ${NUM_CORE} ]
#do
#	echo "CPU ${cpu_no}: ${thread[$cpu_no]} threads"
#	BINARY_NAME="${BINARY_NAME}-${thread[$cpu_no]}threads"
...
#sed -i.bak 's/<THREADS '${thread[0]}'\>/THREADS 1/g' inc/champsim.h
